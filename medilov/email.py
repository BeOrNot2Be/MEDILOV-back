...
def send_appoitment_registration(data):
    ...
    client_msg = EmailMultiAlternatives(
        client_sub, client_text, settings.EMAIL_HOST_USER, [data['email']])
    wh_msg = EmailMultiAlternatives(wh_sub, wh_text, settings.EMAIL_HOST_USER, [
                                    settings.WEBSITEHOLDEREMAIL])
